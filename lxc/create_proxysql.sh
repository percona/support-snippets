...
for (( i=1; i<=$NUMBER_OF_NODES; i++ ))do
  ...
  if [[ "$VERSION" == "1.4" ]]; then
    lxc exec $NODE_NAME -- yum -y install tar gdb strace vim qpress socat wget sudo less perf proxysql Percona-Server-client-57 which
  elif [[ "$VERSION" == "2.0" ]]; then
    lxc exec $NODE_NAME -- yum -y install tar gdb strace vim qpress socat wget sudo less perf proxysql2 Percona-Server-client-57 which
  fi
