...
for (( i=1; i<=$NUMBER_OF_NODES; i++ ))do
    ...
    if [[ ! -z "$VERSION" ]]; then
      lxc exec $NODE_NAME -- yum -y install tar gdb strace vim qpress socat wget sudo $VERSION
      VERSION_ACRONYM=$( echo ${VERSION} | awk -F'-' '{print $4}') #55, 56, 57, 80
    else
      lxc exec $NODE_NAME -- yum -y install tar gdb strace vim qpress socat wget sudo Percona-XtraDB-Cluster-57
      VERSION_ACRONYM="57"
    fi
