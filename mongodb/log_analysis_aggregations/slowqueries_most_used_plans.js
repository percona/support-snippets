db.getSiblingDB('percona').getCollection('log').aggregate([
  ...
  },
  {
    $project:{
      ...
      "top_oplogSlotDurationMillis": {$divide: [ { $arrayElemAt : [ "$top_totalOplogSlotDurationMicros", 0 ]}, 1000]},
      "top_planningTimeMillis": {$divide: [ { $arrayElemAt : [ "$top_planningTimeMicros", 0 ]}, 1000]},
